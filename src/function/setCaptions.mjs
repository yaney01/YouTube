...
function findSourceTrackIndex(captionTracks, sourceLanguage) {
	...
	if (!normalizedSource || normalizedSource === "auto") {
		const autoGeneratedIndex = findAutoGeneratedTrackIndex(captionTracks);
		return autoGeneratedIndex >= 0 ? autoGeneratedIndex : captionTracks.length ? 0 : -1;
	}
...
export default function setCaptions(captions, translationLanguages, hostname, sourceLanguage = "AUTO", targetLanguage = undefined, sourceOnly = false) {
	...
	if (captions?.playerCaptionsTracklistRenderer) {
		...
		let captionTracks = Array.isArray(tracklist.captionTracks) ? tracklist.captionTracks : [];
		let sourceIndex = findSourceTrackIndex(captionTracks, sourceLanguage);
		...
		if (sourceOnly && sourceIndex >= 0) {
			tracklist.captionTracks = [captionTracks[sourceIndex]];
			captionTracks = tracklist.captionTracks;
			sourceIndex = 0;
			preferredIndex = 0;
		}

		if (!sourceOnly && sourceIndex >= 0 && targetLanguage && !isSamePrimaryLanguage(getTrackLanguage(captionTracks[sourceIndex]), targetLanguage)) {
		...
		if (Array.isArray(tracklist.audioTracks)) {
			...
			tracklist.audioTracks = tracklist.audioTracks.map(audio => {
				...
				if (sourceOnly && preferredIndex >= 0) audio.captionTrackIndices = [preferredIndex];
				else if (preferredIndex >= 0 && Array.isArray(audio.captionTrackIndices) && !audio.captionTrackIndices.includes(preferredIndex)) audio.captionTrackIndices.push(preferredIndex);
